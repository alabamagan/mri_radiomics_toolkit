...
def filter_low_var_features(features: pd.DataFrame,
                            threshold: Optional[float] = .95 * (1 - .95),
                            ) -> Tuple[pd.Index, skfs.VarianceThreshold]:
    ...
    """
    var_filter = skfs.VarianceThreshold(threshold=threshold)
...
def preliminary_feature_filtering(features_a: pd.DataFrame,
                                  ...
                                  var_thres: Optional[float] = 0.95 * (1 - 0.95)
    ...
    for features in [features_a, features_b]:
        try:
            ...
            if features is not None:
                logger.error("Diagnostics column generated by PyRadiomics is not found or error occurs.")

    # Drop features with extremely low variance (i.e., most are same value)
    logger.info(f"Dropping features with low variance with threshold {var_thres}...")
    var_feats_a_index, var_filter = filter_low_var_features(features_a, threshold=var_thres)
    ...
    if features_b is not None:
        ...
        var_feats_b_index, _ = filter_low_var_features(features_b, threshold=var_thres)
    ...
    logger.info(f"Dropping features using T-test or ANOVA with p-value threshold: {p_thres}")
    ...
    logger.info(f"Final remaining features: {len(feats_a.index)}")
...
class FeatureSelector(object):
    ...
    def fit_(self,
        ...
        """
        assert all(y.nunique() == 2), "This method is for binary classification only."
